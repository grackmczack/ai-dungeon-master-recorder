#!/usr/bin/env python3
"""Replicate the exact buildPrompt output and call the LLM."""
import json
...
# Exactly what buildPrompt produces: system prompt prepended, then TRANSKRIPT:
TRANSCRIPT_TEXT = """[0:00] Arkele: Und damit starten wir unsere erste aufgenommene Runde.
...
# Print the exact message that would be sent (truncated)
print("=" * 70)
print("USER MESSAGE (first 500 chars):")
print(USER_MESSAGE[:500])
print("...")
print(f"\nTotal message length: {len(USER_MESSAGE)} chars")
print("=" * 70)

# Write the user message to a file for reference
with open("/home/grack/.openclaw/workspace/dnd-recorder/test_user_message.txt", "w") as f:
    f.write(USER_MESSAGE)
print("Full user message written to test_user_message.txt")
